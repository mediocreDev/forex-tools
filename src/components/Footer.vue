<template>
  <footer class="footer footer-center p-10 bg-base-200 text-base-content rounded">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 place-items-start gap-8 w-full max-w-4xl">
      <div v-for="category in footerLinks" :key="category.category" class="text-center">
        <h3 class="font-semibold text-base-content mb-3">{{ category.category }}</h3>
        <nav class="flex flex-col items-center gap-2">
          <component v-for="link in category.links" :key="link.path" :is="isRouterLink(link.path) ? 'router-link' : 'a'"
    ...
    <nav>
      <div class="grid grid-flow-col gap-4">
        <a v-for="social in socialLinks" :key="social.name" :href="social.url" :title="social.name"
          class="link link-hover" target="_blank" rel="noopener noreferrer">
          <component :is="getIconComponent(social.icon)" class="w-6 h-6" />
        </a>
      </div>
    </nav>
    ...
    <aside>
      <p>
        Copyright &copy; {{ currentYear }} - {{ brandConfig.name }}. Americio&trade;
      </p>
